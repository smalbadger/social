...
class PinValidation:
    ...
    def get_pin(self, username:str, email_origin:'str', timeout:timedelta, time:datetime = None) -> str:
        ...
        :param email_origin: The email belonging to the user for which we're validating the pin.
        ...
        #query = f'(FROM "security-noreply@linkein.com" TO "{email_origin}" TEXT "{needed_msg}")'
        query = f'(TO "{email_origin}")'
        #query = f'(FROM "security-noreply@linkein.com" TO "{email_origin}" TEXT "{needed_msg}")'
        #query = f'(FROM "security-noreply@linkein.com" TO "{email_origin}" TEXT "{needed_msg}")'
        #query = f'(FROM "security-noreply@linkein.com" TO "{email_origin}" TEXT "{needed_msg}")'
        ...
        for i in mail_ids:
            ...
            if time_running > timeout:
                raise Exception("Timeout while searching for the pin validation email")
            ...
            for response_part in data:
                ...
                if isinstance(response_part, tuple):
                    ...
                    if needed_msg in mail_content and username in mail_content:
...
if __name__ == "__main__":
    ...
    pin = PinValidation().get_pin("ÔỐỒỔỖỘÔỐỒỔỖỘôốồổỗộôố ƯỨỪỬỮỰƯỨỪỬỮỰưứừửữựưứ", "linkedin.test11@facade-technologies.com", timeout)
    print(pin)